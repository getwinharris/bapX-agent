...
|------|--------|------------|
| OAuth token | `***` | `hermes model` → GitHub Copilot → Login with GitHub |
| Fine-grained PAT | `***` | GitHub Settings → Developer settings → Fine-grained tokens (needs **Copilot Requests** permission) |
...
Hermes sends a supported GitHub token (`****`, `****`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).
